...
def analyze_sheet_data(
    ...
    _validate_basic_compilation_fields(raw_data, file_type, add_error)
    ...
    _apply_dynamic_validation_rules(raw_data, tipologia_strumento_scheda, modello_l9_scheda_normalizzato, add_error)

    # Applica la logica di validazione hardcoded
    if file_type and tipologia_strumento_scheda != "N/D":
        _validate_hardware_specific_rules(
            raw_data, file_type, tipologia_strumento_scheda,
            modello_l9_scheda_normalizzato, human_errors, add_error
        )

    # Validazione Certificati
    extracted_certs_data = []
    if file_type:
        extracted_certs_data = _process_certificates_usages(
            raw_data, file_type, base_filename, file_path, card_date,
            tipologia_strumento_scheda, modello_l9_scheda_normalizzato,
